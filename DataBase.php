...
include "class/class_categorie.php" ;
include "class/class_Panier.php" ;
include "class/class_detailsCommande.php" ;
...
class Users extends Database {
    ...
    public function CreateUser($User)  {
        ...
        $usersData = $this->insertData('users', ["name", "prénom", "phone", "Email", "adresse", "ville", "Password", "is_Active"], [$User->getName(),$User->getPrenom(),$User->getPhone(),$User->getEmail(),$User->getAdresse(),$User->getVille(),$User->getPassword(),$User->getIsActive()] ); 
...
class Paniers extends Database {
    ...
    public function getPanier( $columns = '*', $where = '', $orderBy = '', $limit = 'all') {
        ...
        $PanierData = $this->selectData('panier', $columns, $where , $orderBy , $limit ); 
        // echo "<pre>";
        // print_r($PanierData);
        // echo "</pre>";
        ...
        foreach ($PanierData as $B) {
            $Panier[] = new Panier($B["panier_id"],$B["Etiquette"],$B["img"],$B["OffreDePrix"],$B["QuantiteStock"],$B["Stock"],$B["client_id"],$B["valid_commend"]);        } 
        return $Panier;
...
class DetailsCommandes extends Database {
    ...
    public function getDetailsCommandes( $columns = '*', $where = '', $orderBy = '', $limit = 'all') {
        ...
        foreach ($DetailsCommandesData as $B) {
            $DetailsCommandes[] = new DetailsCommande($B["details_id"],$B["names"],$B["prix_total"],$B["commande_id"],$B["id_user"],$B["confirm_achter"],$B["created_at"]);      
          } 
